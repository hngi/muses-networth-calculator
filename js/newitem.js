...
const applyMode = () => {
  ...
  document.getElementById("intro-spec").textContent =
    (editItem ? "Edit " : "Add ") + (mode === "ASSET" ? "Asset" : "Liability");
  document.getElementById("item-type").value = mode.toLowerCase();
  document
    .getElementById("item-value")
    .setAttribute(
      "placeholder",
      mode === "ASSET" ? "Asset value" : "Liability value"
    );
  ...
  if (editItem) {
    ...
    document.getElementById("newitem-info-main").textContent =
      "Changes applied";
  }
};
...
document.getElementById("newitem-submit").addEventListener("click", e => {
  ...
  if (!value.length || !description.length) return;
