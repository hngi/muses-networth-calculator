...
document.querySelector(
  "#overview-controls-sort select"
).value = localStorage.getItem(overview_sort);
...
const powerUp = () => {
  ...
  fetch(backendUrl + "/api/items", {
    ...
    .then(data => data.json())
    .then(json => {
      if (json.code) console.log(`Error! ${json.code}: ${json.errors}`);
      else {
        items = json;
        populateTable(items);
      }
  ...
  document.getElementById("overview-info").textContent =
    "Items loading. Please wait. . .";

  switch (mode) {
    case "ASSET":
      document
        .getElementById("liabilities-switch")
        .classList.remove("item-switch-active");
      document
        .getElementById("assets-switch")
        .classList.add("item-switch-active");
    ...
    case "LIABILITY":
      ...
      document
        .getElementById("assets-switch")
        .classList.remove("item-switch-active");
      document.getElementById("overview-add-item").textContent =
        "Add Liability";
      break;
  }
};
...
const updateControlFocus = () => {
  ...
  if (selectedItems.length) {
    ...
    del.setAttribute(
      "title",
      "Delete item" + (selectedItems.length > 1 ? "s" : "")
    );
  } else {
    del.classList.remove("overview-controls-focus");
    del.setAttribute(
      "title",
      "You must select one or more rows in order to delete"
    );
  }
...
const selectRow = e => {
  ...
  if (cb.checked) selectedItems.push(items.find(a => cb.id == a.id));
  else selectedItems = selectedItems.filter(a => cb.id != a.id);
  document.getElementById("overview-select-all").checked = false;
  updateControlFocus();
};
...
const populateTable = items => {
  ...
  let filteredItems = search.length
    ? sortedItems.filter(a => a.description.toLowerCase().includes(search))
    : sortedItems;
  ...
  while (table.childNodes[2]) table.removeChild(table.childNodes[2]);

  table.appendChild(th);
  for (let item of filteredItems) {
    if (item.type.toLowerCase() !== mode.toLowerCase()) continue;
  ...
  if (!displayedCBs.length) {
    ...
    document.getElementById(
      "overview-info"
    ).textContent = document.getElementById("overview-search").value.length
      ? `You have no ${
          mode === "ASSET" ? "assets" : "liabilities"
        }  matching that description`
      : `You have not added any ${mode === "ASSET" ? "assets" : "liabilities"}`;
  } else document.getElementById("overview-info").style.display = "none";
};

document.getElementById("assets-switch").addEventListener("click", e => {
  localStorage.setItem(
    overview_mode,
    localStorage.getItem(overview_mode) === "ASSET" ? "LIABILITY" : "ASSET"
  );
...
document.getElementById("overview-select-all").addEventListener("click", e => {
  ...
  for (let cb of displayedCBs) {
    ...
    if (e.target.checked) selectedItems.push(items.find(a => cb.id == a.id));
  }
  updateControlFocus();
});

document
  .getElementById("overview-controls-edit")
...
document
  .getElementById("overview-controls-delete")
  .addEventListener("click", e => {
    e.preventDefault();
    if (!selectedItems.length) return;

    document.getElementById("overview-modal-container").style.display = "block";
    document.getElementById("overview-delete-query").textContent =
      "Are you sure you want to delete " +
      (selectedItems.length > 1 ? "these items?" : "this item?");
    document.addEventListener("keydown", e => {
      if (
        document.getElementById("overview-modal-container").style.display ==
        "block"
      ) {
        if (e.code == "Enter") deleteItem();
        else e.code == "Escape";
        document.getElementById("overview-modal-container").style.display =
          "none";
      }
    ...
    document
      .getElementById("overview-delete-yes")
      .addEventListener("click", e => {
        deleteItem();
      });
    document
      .getElementById("overview-delete-cancel")
      .addEventListener("click", e => {
        document.getElementById("overview-modal-container").style.display =
          "none";
      });
    document
      .getElementById("overview-modal-container")
      .addEventListener("click", e => {
        if (
          e.path &&
          e.path[0] == document.getElementById("overview-modal-container")
        )
          document.getElementById("overview-modal-container").style.display =
            "none";
      });
...
document
  .querySelector("#overview-controls-sort select")
...
document.getElementById("overview-calculate").addEventListener("click", e => {
  localStorage.setItem(
    asset_value,
    items.reduce(
      (total, a) => total + (a.type == "asset" ? parseInt(a.value) : 0),
      0
    )
  );
  localStorage.setItem(
    liability_value,
    items.reduce(
      (total, a) => total + (a.type == "liability" ? parseInt(a.value) : 0),
      0
    )
  );
  console.log(items);
