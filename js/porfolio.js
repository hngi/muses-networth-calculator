...
const getPercentile = n => {
  ...
  if (dollar >= 100000000) return 95;
  if (dollar >= 10000000) return 93;
  if (dollar >= 1000000) return 92;
  if (dollar >= 100000) return 91;
  if (dollar < 5) return 1;
  if (dollar < 10) return 2;
  if (dollar < 20) return 3;
  let index = percentiles.indexOf(percentiles.find(n => n > dollar));
  if (index == -1 || index == 19) return 98;
  return (index + 1) * 5;
};

document.getElementById("stats-assets-value").textContent = convertToString(
  asset
);
document.getElementById(
  "stats-liabilities-value"
).textContent = convertToString(liability);
document.getElementById("stats-networth-value").textContent =
  "₦" + convertToString(networth);
let percentile = getPercentile(networth).toString();
let pString = percentile;
if (percentile[percentile.length - 1] == "1") pString += "st";
else if (percentile[percentile.length - 1] == "2") pString += "nd";
else if (percentile[percentile.length - 1] == "3") pString += "rd";
else pString += "th";
